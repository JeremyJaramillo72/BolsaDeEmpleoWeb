...
public class NotificacionController {
    ...
    public ResponseEntity<List<NotificacionDTO>> obtenerMisNotificaciones(@PathVariable Long idUsuario) {
        try {
            // Verificar si hay ofertas pendientes y crear notificación si corresponde (solo para admins)
            try {
                notificacionService.verificarYNotificarOfertasPendientes(idUsuario);
            } catch (Exception e) {
                log.warn("Error verificando ofertas pendientes para usuario {}: {}", idUsuario, e.getMessage());
            }
